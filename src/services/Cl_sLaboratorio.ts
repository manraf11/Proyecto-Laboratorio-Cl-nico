...
// Definir interfaz para los items de la reducción
interface PrecioItem {
    precio: number;
}

export default class Cl_sLaboratorio {
    static obtenerUsuarioSesion(): { id: number; nombre: string } {
        try {
    ...
    static async actualizarTotalExamen(examenId: number): Promise<void> {
        try {
            ...
            const total = data?.reduce((sum: number, item: PrecioItem) => sum + (item.precio || 0), 0) || 0;
    ...
    static async buscarPorCedula(cedula: string): Promise<{ ok: boolean; registro?: any; nombreApi?: string }> {
        try {
